...
import { getCurrenciesIsoCodeHistory, getGetCurrenciesIsoCodeHistoryQueryKey } from '@/modules/api/codegen';
...
export default function CurrencyVersionHistory() {
    ...
    const query = useQuery({
        queryKey: [getGetCurrenciesIsoCodeHistoryQueryKey({ isoCode: stringIsoCode })],
        queryFn: async () => {
            const response = await getCurrenciesIsoCodeHistory({ isoCode: stringIsoCode });
                                    ...
                                    <div className='flex items-center gap-4'>
                                        ...
                                        <div>
                                            ...
                                            <p className='text-slate-300'>
                                                {data.newestVersion?.name} ({data.newestVersion?.isoCode}) - Complete
                                                change history
                                            </p>
                                        </div>
                                    </div>
                                    ...
                                    <CardContent className='px-6'>
                                        <div className='grid grid-cols-1 md:grid-cols-5 gap-4'>
                                            ...
                                            <div>
                                                ...
                                                <Badge
                                                    className={
                                                        data.newestVersion.isCurrent ? 'bg-green-600' : 'bg-red-600'
                                                    }
                                                >
                                    ...
                                    <Card key={version.id} className='bg-slate-800/90 border-slate-700'>
                                        <CardContent className='px-6'>
                                            ...
                                            <div className='grid grid-cols-1 md:grid-cols-5 gap-4 p-4 bg-slate-700/30 rounded-lg border border-slate-600'>
                                                ...
                                                <div>
                                                    <p className='text-slate-400 text-sm mb-1'>Symbol</p>
                                                    <p className='text-cyan-400 font-bold text-lg'>{version.symbol}</p>
                                                </div>
                                                <div>
